...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(rclcpp::Node::SharedPtr &node)
  {
    ...
    node_->declare_parameter("fsm/replan_from_odom_on_tracking_error", true);
    node_->declare_parameter("fsm/odom_replan_tracking_error", 0.05);
    ...
    node_->get_parameter("fsm/replan_from_odom_on_tracking_error", replan_from_odom_on_tracking_error_);
    node_->get_parameter("fsm/odom_replan_tracking_error", odom_replan_tracking_error_);
  ...
  void EGOReplanFSM::BroadcastBsplineCallback(const std::shared_ptr<const traj_utils::msg::Bspline> &msg)
  {
    ...
    rclcpp::Clock &clock = *node_->get_clock();  // 确保使用当前节点的时间源
    ...
    if (abs((clock.now() - msg_time).seconds()) > 0.25)
    {
      ...
      RCLCPP_ERROR(node_->get_logger(), "Time difference is too large! Local - Remote Agent %d = %fs",
                   msg->drone_id, (clock.now() - msg_time).seconds());
      return;
    }
  ...
  void EGOReplanFSM::execFSMCallback()
  {
    ...
    data_disp_.header.stamp = clock.now();
  ...
  void EGOReplanFSM::checkCollisionCallback()
  {
    ...
    double t_cur = (clock.now() - info->start_time_).seconds();
    ...
    double t_cur_global = clock.now().seconds();
